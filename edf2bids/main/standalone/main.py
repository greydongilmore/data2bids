...
class MainWindow(QtWidgets.QMainWindow, gui_layout.Ui_MainWindow):
	def __init__(self):
		...
		self.updateStatus("Welcome to EpLink EDF converter. Load your directory.")
		...
		if dialog.exec_():
			...
			for i in range(parent_count):
				...
				for j in range(child_count):
					...
					for ientry in range(len(self.file_info[sub])):
						for irun in range(len(self.file_info[sub][ientry])):
							if self.file_info[sub][ientry][irun]['DisplayName'] == item.text(0):
								if self.file_info[sub][ientry][irun]['SamplingFrequency'] != int(item.text(4)):
									self.file_info[sub][ientry][irun]['SamplingFrequency'] = int(item.text(4))
									self.file_info[sub][ientry][irun]['TotalRecordTime'] = round((((self.file_info[sub][ientry][irun]['NRecords']*(int(item.text(4))*self.file_info[sub][ientry][irun]['RecordLength']))/int(item.text(4)))/60)/60,3)
								
								if self.file_info[sub][ientry][irun]['RecordingType'] != self.treeViewLoad.itemWidget(item, 7).currentText():
									self.file_info[sub][ientry][irun]['RecordingType'] = self.treeViewLoad.itemWidget(item, 7).currentText()
								
								if self.file_info[sub][ientry][irun]['RecordingLength'] != self.treeViewLoad.itemWidget(item, 8).currentText():
									self.file_info[sub][ientry][irun]['RecordingLength'] = self.treeViewLoad.itemWidget(item, 8).currentText()
							
								if self.file_info[sub][ientry][irun]['Retro_Pro'] != self.treeViewLoad.itemWidget(item, 9).currentText():
									self.file_info[sub][ientry][irun]['Retro_Pro'] = self.treeViewLoad.itemWidget(item, 9).currentText()
