...
class edf2bids(QtCore.QThread):
		...
		if overwrite:
			for iblock in range(len(overwrite)):
				if 'Montage' in overwrite[iblock][0]:
					...
					text_update = text_update.replace(header[0]['firstname'], 'firstname')
					text_update = text_update.replace(header[0]['lastname'], 'lastname')
	...
	def _annotations_data(self, file_info_run, annotation_fname, data_fname, source_name, overwrite, verbose):
		...
		header[0]['firstname'] = None
		header[0]['lastname'] = None
		if '~' in filen:
			firstname = filen.split('~')[1].strip().lower()
			header[0]['firstname'] = firstname if firstname != 'x' else None
			lastname = filen.split('~')[0].strip().lower()
			header[0]['lastname'] = lastname if lastname != 'x' else None
