			...
			try:
				...
				if not chan_label_file_ses:
					if chan_label_filename:
						...
						chan_label_file = header[1]['ch_names']
				...
				ch_info[chan_type] = OrderedDict([
					...
					('Unit', np.array(header[1]['units'])[eeg_chan_idx]),
...
# raw_file_path = r'C:\Users\Greydon\Downloads\Input'
# output_path = r'C:\Users\Greydon\Downloads\test_output'
...
#									 'offset_dates': False}
# input_dir = r'C:\Users\Greydon\Downloads\Input'
				...
				for ises in range(len(values)):
					...
					if '_SE' in values[ises][0]['DisplayName']:
						if ''.join([date_study,'T', values[ises][0]['Time']]) not in scans_tsv['acq_time'].values:
							session_labels.append('ses-' + ''.join(values[ises][0]['DisplayName'].split('_')[3:6]))
	...
	with open(data_fname, 'rb') as fid:
		...
		for i in range(header[0]['nchan']):
			...
			if i == chanindx:
				...
				if len(raw)>2:
					if header[0]['firstname'] is not None:
						...
						elif any(x.lower() in raw[2].lower() for x in {header[0]['firstname'], header[0]['lastname']}):
							 fid_loc = [(np.int64(header[0]['data_offset']) + np.int64(block) * np.int64(blocksize)) + read_idx, np.int64(header[1]['n_samps'][i]*header[0]['data_size'])]
...
def deidentify_edf(fname, isub, offset_date, rename):
	...
	if offset_date:
		...
		date_study = datetime.datetime.strptime(datetime.datetime.strptime(header[0]['meas_date'], '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d'), '%Y-%m-%d')
